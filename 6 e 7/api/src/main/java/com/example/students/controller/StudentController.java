...
public class StudentController {
    ...
    public ResponseEntity<List<Double>> getAllTestsByStudentId(@PathVariable(value = "studentId") Long studentId){
        ...
        Double maximun = (tests.size() > 0) ? Math.round(testService.maximunGradeByStudentId(studentId) * 100.0)/100.0 : 0;
        Double minimun = (tests.size() > 0) ? Math.round(testService.minimunGradeByStudentId(studentId) * 100.0)/100.0 : 0;
        Double average = (tests.size() > 0) ? Math.round(testService.gradeAverageByStudentId(studentId) * 100.0)/100.0 : 0;
        Double tests_number = (tests.size() > 0) ? Double.valueOf(tests.size()) : 0;
